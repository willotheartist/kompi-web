...
import { ArrowUpRight, BookOpen, Link2, BarChart3 } from "lucide-react";
...
const CANONICAL = "/blog/utm";

export const metadata: Metadata = {
  title: "UTM Builder — Guides, Examples, Tracking | Kompi",
  description:
    "Practical UTM guides, naming rules, and examples — plus how to build, shorten, and track UTMs with Kompi.",
...
function pickCoverFor(p: { slug: string; primaryKeyword: string }) {
  ...
  if (s.includes("analytics") || s.includes("track") || s.includes("measure")) return "/growth/analytics.png";
  if (s.includes("short") || s.includes("url")) return "/growth/links.png";
  return "/growth/subscribers.png";
}

export default function UTMHubPage() {
  const pages = getAllPSEOPages()
    .filter((p) => p.cluster === "utm")
          ...
          <Container>
            <div className="flex flex-col gap-5">
              ...
              <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight text-neutral-950">
                UTMs that stay consistent.
              </h1>

              <p className="max-w-2xl text-base md:text-lg leading-relaxed text-neutral-700">
                A practical library of UTM naming rules, examples, and tracking tips — so you can measure what’s
                working and stop guessing.
              </p>
              ...
              <div className="mt-2 flex flex-col sm:flex-row gap-3">
                ...
                <Link
                  ...
                  className="group inline-flex items-center justify-center gap-2 rounded-2xl border border-black/10 bg-white px-5 py-3 font-semibold shadow-sm hover:shadow-lg transition"
                >
                  <BarChart3 className="h-4 w-4 opacity-80 group-hover:opacity-100 transition" />
                  View analytics
                  <ArrowUpRight className="h-4 w-4 opacity-70 group-hover:opacity-100 transition" />
                </Link>
              </div>
          ...
          <Container>
            {featured ? (
              ...
              >
                ...
                <div className="md:col-span-5 p-7 md:p-10 flex flex-col justify-between gap-6">
                  <div className="space-y-3">
                    <div className="text-sm font-semibold text-neutral-700">Featured UTM guide</div>
                    <h2 className="text-2xl md:text-3xl font-extrabold tracking-tight text-neutral-950">
                      {featured.primaryKeyword}
                    </h2>

                    <p className="text-neutral-700 leading-relaxed">
                      {featured.intent ? `Intent: ${featured.intent}.` : "Naming rules, examples, and tracking tips."}{" "}
              ...
              <div className="rounded-3xl border border-black/10 bg-[#F7F7F4] p-10 text-center">
                <div className="text-xl font-bold text-neutral-950">No UTM posts found</div>
                <div className="mt-2 text-neutral-700">
                  Your dataset returned no pages in the{" "}
                  <code className="px-2 py-1 rounded bg-white border border-black/10">utm</code> cluster.
                </div>
              </div>
            )}
          </Container>
          ...
          <section className="pb-16 md:pb-24">
            <Container>
              <div className="flex items-end justify-between gap-6 mb-6">
                <div>
                  <h2 className="text-2xl font-extrabold tracking-tight text-neutral-950">All UTM articles</h2>
                  <p className="mt-1 text-neutral-700">Browse templates, naming rules, and tracking playbooks.</p>
                </div>
              </div>
                ...
                {rest.map((p) => (
                  ...
                  >
                    ...
                    <div className="p-6 md:p-7 space-y-3">
                      <div className="text-sm font-semibold text-neutral-700">{p.intent ?? "UTM"}</div>
