...
function YourKRCodesPageInner() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const res = await fetch("/api/kr-codes", { credentials: "include" });
        ...
        console.error(err);
        if (!cancelled) setError("Failed to load Kompi Codes");
      } finally {
  ...
  async function handleDelete(code: KRCode) {
    ...
    try {
      ...
      setDeleteIntentFor((current) => (current === code.id ? null : current));
    }
  }

  const totalScans = codes.reduce((sum, c) => sum + (c.totalClicks ?? 0), 0) ?? 0;
      ...
      <header className="mb-1">
        ...
        <p className="mt-1 text-[13px] text-[#6B7280]">
          ...
          {workspaceName ? (
            <>
              {" "}
              in <span className="font-medium text-[#111827]">{workspaceName}</span>.
            </>
          ) : (
            "."
          )}
        ...
        <p className="mt-1 text-[11px] text-[#9CA3AF]">
          Total codes:{" "}
          <span className="font-medium text-[#111827]">{codes.length}</span> · Total scans:{" "}
          <span className="font-medium text-[#111827]">{totalScans}</span>
        </p>
      </header>
        ...
        {loading ? (
          <div className="space-y-4">
            ...
            {Array.from({ length: 3 }).map((_, i) => (
              <Skeleton key={i} className="h-24 w-full rounded-4xl bg-[#E5E7EB]" />
            ))}
          </div>
        ) : error ? (
          ...
          <div className="space-y-3">
            {codes.map((code) => {
              ...
              return (
                <div
                  key={code.id}
                  className="flex flex-col gap-4 rounded-4xl bg-white px-4 py-4 md:flex-row md:items-center md:justify-between md:px-6 md:py-5"
                >
                  <div className="flex flex-1 items-center gap-4 md:gap-6">
                    ...
                    <div className="min-w-0 space-y-1">
                      ...
                      <div className="truncate text-[13px] text-[#4C6FFF]">{code.destination}</div>
                      <div className="flex flex-wrap items-center gap-4 text-[12px] text-[#6B7280]">
                        <span>
                          Type:{" "}
                          <span className="font-medium">{labelForContentType(code.type)}</span>
                        </span>
                  ...
                  <div className="flex flex-col items-stretch gap-2 text-[13px] md:flex-row md:items-center md:gap-6">
                    <div className="flex items-center justify-between gap-2 md:flex-col md:items-start">
                      <span className="text-[13px] text-[#6B7280]">Total scans</span>
                    ...
                    <div className="flex flex-wrap items-center justify-end gap-3">
                      ...
                      <button
                        ...
                        onClick={() =>
                          setDeleteIntentFor((current) => (current === code.id ? null : code.id))
                        }
                      ...
                      <div className="relative">
                        <button
                          type="button"
                          onClick={() =>
                            setDownloadMenuFor((current) => (current === code.id ? null : code.id))
                          }
                        ...
                        {isMenuOpen && (
                          <div
                            ...
                            onMouseLeave={() => setDownloadMenuFor(null)}
                          >
                            <Link
